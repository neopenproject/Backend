...
class AnswerPost(Base):
    ...
    is_over = Column(Boolean, default=False)  # 시간 초과 여부
    author = Column(String(length=128))
    is_grade_view = Column(Boolean, default=False)  # 채점 확인 여부
    score = Column(Integer)  # 강사 점수
    problem_post = Column(Integer)  # 문제관계
    comment = Column(String(length=512))  # 강사 커맨트
    is_teacher_view = Column(Boolean, default=False)  # 강사 확인 여부
    is_grade = Column(Boolean, default=False)  # 채점 여부
